...
Patcher ThrottlePatcher("Wheel: Throttle", throttle, true);

// When disabled, brake pressure doesn't decrease.
PatternInfo brake;
Patcher BrakePatcher("Wheel: Brake", brake, true);
...
Patcher ThrottleControlPatcher("Throttle: Throttle control", throttleControl, true);
...
bool ApplyGearboxPatches() {
    ...
    logger.Write(LogLevel::ERRORs, "[Patch] [Gears] Patching failed");
    gearboxAttempts++;

    if (gearboxAttempts > maxAttempts) {
        logger.Write(LogLevel::ERRORs, "[Patch] [Gears] Patch attempt limit exceeded");
        logger.Write(LogLevel::ERRORs, "[Patch] [Gears] Patching disabled");
    }
    return false;
}
...
bool RevertGearboxPatches() {
    ...
    logger.Write(LogLevel::ERRORs, "[Patch] [Gears] Restore failed");
    return false;
}
